${MKDIR} /work/mounts &&
  ...
  do
    NAME=$( ${BASENAME} "$FILE" ) &&
      ...
      ${LN} --symbolic ${OUT}/bin/initial.${HASH}.sh /work/mounts/${HASH}.sh &&
      ...
      then
        ${ECHO} non-empty standard-output for ${HASH} >> /work/ERROR &&
          ${CAT} /work/mounts/${HASH}.standard-output >> /work/ERROR
      elif [ ! -e /work/mounts/${HASH}.standard-error ]
      then
        ${ECHO} missing standard-error for ${HASH} >> /work/ERROR
      elif [ ! -z "$( ${CAT} /work/mounts/${HASH}.standard-error ) }" ]
      then
        ${ECHO} non-empty standard-error for ${HASH} >> /work/ERROR &&
          ${CAT} /work/mounts/${HASH}.standard-error >> /work/ERROR
      ...
      elif [ $( ${FIND} /work/mounts/${HASH}/mount -mindepth 1 -maxdepth 1 ! -name target | ${SORT} | ${WC} --lines ) != 0 ]
      then
